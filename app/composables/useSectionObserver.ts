...
export function useSectionObserver(
  sectionRefs: Ref<ComponentPublicInstance | null>[] = [],
) {
      ...
      if (relevantEntries.length > 0) {
        const bestEntry = relevantEntries.reduce((best, current) => {
          ...
          const currentCenterDistance
            = Math.abs((currentRect.top + currentRect.bottom) / 2 - window.innerHeight / 2);
          const bestCenterDistance
            = Math.abs((bestRect.top + bestRect.bottom) / 2 - window.innerHeight / 2);
